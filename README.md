...
| 問題 | 公開版的處理方法 |
|---|---|
| 同一人短時間內有多次錄音，逐列最近時間配對可能重複使用同一筆 | 在同一受試者、同日的候選集合內，最大化容差內的一對一配對數，再最小化總時間差 |
| 日期只有「某一天」，或有數個同樣好的答案 | 不補猜午夜；日期缺少時間或存在等價最優配對時標記供人工確認 |
| 下載到登入頁、截斷 WAV 或異常大檔案 | HTTP 狀態、HTML 類型、檔案大小、音訊標頭與 PCM WAV 結構檢查；驗證後才發布檔案 |
...
其餘案例刻意包含：超過時間容差、日期缺少時間、等價配對，以及假裝是 WAV 的錯誤頁面。這些是**合成測試結果，不是研究樣本數或模型成績**。
